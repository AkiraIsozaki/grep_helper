...
- `classify_usage_regex()`: 7種の使用タイプのサンプルコード
- `extract_variable_name()`: 各種変数定義パターン
- `write_tsv()`: 出力ファイルの列数・エンコード・ソート順

### 統合テスト
- サンプルJavaソースファイル群 + サンプルgrep結果を用いたE2Eフロー
- 直接参照・間接参照・getter経由参照が全て正しくTSVに出力されること
